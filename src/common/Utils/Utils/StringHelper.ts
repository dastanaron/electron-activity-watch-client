export default class StringHelper
{
    ...
    public cropText(text: string, maxLength: number = 500, trailingCharacters: string = '...'): string
    {
    ...
    public numberToStringForm(number: number, textForms: string[]): string
    {
        ...
        if (absoluteNumber > 10 && absoluteNumber < 20) { return textForms[2]; }
        if (latestNumber > 1 && latestNumber < 5) { return textForms[1]; }
        if (latestNumber == 1) { return textForms[0]; }
        return textForms[2];
    }
}
