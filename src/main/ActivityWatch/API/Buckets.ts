...
export default class Buckets {
    ...
    public loadAllBuckets(): Promise<any> {
        return fetch(this.addressDeclarator.getUrl(), { method: METHOD_GET }).then((res) => res.json());
    }
    ...
    public createBucket(
        ...
        bucketHostname: string = DEFAULT_BUCKET_HOSTNAME,
    ): Promise<any> {
    ...
    public deleteBucketById(bucketId: string = DEFAULT_BUCKET_ID, force = 1): Promise<any> {
    ...
    public loadBucketMetaByID(bucketId: string = DEFAULT_BUCKET_ID): Promise<any> {
        return fetch(`${this.addressDeclarator.getUrl()}${bucketId}`, { method: METHOD_GET }).then((res) => res.json());
    }

    public pushHeartBeat(
        timeStamp: Date,
        duration: number,
